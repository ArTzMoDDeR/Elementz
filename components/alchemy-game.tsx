...
function GuestWallOverlay({ lang }: { lang: string }) {
  ...
  const [loadingProvider, setLoadingProvider] = useState<'apple' | 'google' | 'discord' | null>(null)

  const handleSignIn = (provider: 'apple' | 'google' | 'discord') => {
        ...
        <div className="flex flex-col gap-2 w-full">
          ...
          >
            {loadingProvider === 'apple' ? (
              <svg className="w-4 h-4 animate-spin flex-shrink-0" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
            ) : (
          ...
          <div className="flex items-center gap-2 my-0.5">
            ...
            <span className="text-xs text-muted-foreground/60">{t('ou par email', 'or by email')}</span>
            <div className="flex-1 h-px bg-border" />
          </div>
