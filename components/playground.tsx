...
function CodexPanel({
  ...
  // Filter by search (only among discovered)
    ...
    <div className="flex flex-col gap-4 py-1">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-bold text-foreground">{t('Codex', 'Codex')}</h2>
      ...
      {/* Recipe detail — shown when an element is selected */}
      {selectedEl && (
        <div
          ...
          <div className="flex items-center gap-4 px-4 py-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {t('Élément débloqué', 'Unlocked element')}
              </p>
            </div>
            <button
              ...
              <>
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    ...
                    style={{ borderColor: `${ingA.color}40`, background: `${ingA.color}15` }}
                  >
                    {ingA.imageUrl
                      ? <img src={ingA.imageUrl} alt={ingA.name} className="w-full h-full object-contain" draggable={false} />
                      : <span className="text-sm font-bold text-white/60">{ingA.name[0].toUpperCase()}</span>
                    }
                  </div>
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    className="w-11 h-11 rounded-xl flex items-center justify-center overflow-hidden p-1 border"
                    style={{ borderColor: `${ingB.color}40`, background: `${ingB.color}15` }}
                  >
                    {ingB.imageUrl
                      ? <img src={ingB.imageUrl} alt={ingB.name} className="w-full h-full object-contain" draggable={false} />
                      : <span className="text-sm font-bold text-white/60">{ingB.name[0].toUpperCase()}</span>
                    }
                  </div>
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    ...
                    style={{ borderColor: `${selectedEl.color}70`, background: `${selectedEl.color}20` }}
                  >
                    {selectedEl.imageUrl
                      ? <img src={selectedEl.imageUrl} alt={selectedEl.name} className="w-full h-full object-contain" draggable={false} />
                      : <span className="text-sm font-bold text-white/80">{selectedEl.name[0].toUpperCase()}</span>
                    }
                  </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {filtered.map(name => {
          ...
          if (!isDiscovered) {
            return (
              <div
                key={name}
                className="aspect-square rounded-xl flex flex-col items-center justify-center border border-border/50 bg-muted/30"
              >
          ...
          return (
            <button
              ...
              className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-1 border transition-all active:scale-95 overflow-hidden p-1.5 ${
                isSelected
                  ? 'border-2 shadow-sm'
                  : 'border hover:opacity-80'
              }`}
              style={{
                borderColor: isSelected ? `${el.color}80` : `${el.color}25`,
                background: isSelected ? `${el.color}18` : `${el.color}0a`,
              }}
            >
              ...
              <span
                ...
                style={{ color: isSelected ? el.color : 'var(--muted-foreground)' }}
              >
                {el.name}
              </span>
            </button>
        ...
        {/* Locked placeholder cards for undiscovered (only when no search) */}
        {!codexSearch && (() => {
          const undiscoveredCount = totalElements - discoveredCount
          // Show up to 12 locked slots as a visual teaser
          const slots = Math.min(undiscoveredCount, 12)
          return Array.from({ length: slots }).map((_, i) => (
            <div
              key={`locked-${i}`}
              className="aspect-square rounded-xl flex flex-col items-center justify-center border border-border/30 bg-muted/20"
            >
