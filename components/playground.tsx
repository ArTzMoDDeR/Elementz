        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {activeTab === 'home' ? (
            ...
            <div className="px-4 py-4">
              ...
              {activeTab === 'help' && (
                <HelpPanel lang={lang} onSetLang={onSetLang} />
              )}
...
function HelpPanel({ lang, onSetLang }: { lang: 'fr' | 'en'; onSetLang: (l: 'fr' | 'en') => void }) {
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-2.5 items-center">
        <p className="text-xs text-muted-foreground text-center leading-relaxed">
          {lang === 'fr'
            ? "Les noms des éléments s'affichent dans la langue choisie."
            : 'Element names appear in the chosen language.'}
        </p>
        <div className="flex items-center bg-muted/50 border border-border rounded-xl p-1 h-10 gap-1">
          <button
            className={`px-6 h-full text-xs font-semibold rounded-lg transition-colors ${lang === 'fr' ? 'bg-background shadow text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            onClick={() => onSetLang('fr')}
          >Français</button>
          <button
            className={`px-6 h-full text-xs font-semibold rounded-lg transition-colors ${lang === 'en' ? 'bg-background shadow text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-start gap-3 p-3 rounded-xl bg-muted/40 border border-border">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              {lang === 'fr'
                ? 'Bouton en haut à droite du terrain pour retirer tous les éléments.'
                : 'Button top-right of the canvas to remove all elements at once.'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 items-center pb-2">
        ...
        <a
          href="/login"
          className="flex items-center gap-2 h-10 px-5 rounded-xl bg-foreground text-background text-xs font-semibold hover:opacity-90 transition-opacity"
        >
