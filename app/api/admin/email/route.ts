...
type Block = { type: string; content?: string; url?: string; label?: string; href?: string; color?: string; align?: string }

// POST — send newsletter with blocks, {username}, and unsubscribe link
export async function POST(req: NextRequest) {
  const session = await auth()
  const sql = neon(process.env.DATABASE_URL!)
  if (!(await checkAdmin(session, sql))) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const { to, subject, blocks = [] } = await req.json() as {
    to: string[]; subject: string; blocks: Block[]
  }
  ...
  for (const user of rows) {
    ...
    const finalSubject = sub(subject)

    const renderBlock = (b: Block): string => {
      const align = b.align ?? 'left'
      switch (b.type) {
        case 'heading': return `<h2 style="margin:0 0 20px;font-size:22px;font-weight:800;color:#e5e5e5;text-align:${align};letter-spacing:-0.3px;">${sub(b.content ?? '')}</h2>`
        case 'text': return `<p style="margin:0 0 16px;line-height:1.75;font-size:15px;color:#c0c0c0;text-align:${align};">${sub(b.content ?? '').split('\n').join('<br/>')}</p>`
        ...
        case 'button': return `<div style="text-align:${align};margin:0 0 20px;"><a href="${sub(b.href ?? '#')}" style="display:inline-block;padding:12px 28px;background:${b.color ?? '#ffffff'};color:#000;font-weight:700;font-size:14px;border-radius:10px;text-decoration:none;">${sub(b.label ?? 'Cliquer ici')}</a></div>`
...
<html lang="fr">
<head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>${finalSubject}</title></head>
...
<tr><td style="padding:28px 36px 24px;border-bottom:1px solid #2a2a2a;"><span style="font-size:18px;font-weight:800;color:#e5e5e5;letter-spacing:-0.3px;">Elementz</span></td></tr>
<tr><td style="padding:28px 36px;">${bodyHtml}</td></tr>
<tr><td style="padding:20px 36px 28px;border-top:1px solid #2a2a2a;">
<p style="margin:0 0 6px;font-size:12px;color:#555;line-height:1.6;">Tu reçois cet e-mail car tu as un compte sur <a href="${APP_URL}" style="color:#777;text-decoration:underline;">elementz.fun</a>.</p>
<p style="margin:0;font-size:12px;"><a href="${unsubUrl}" style="color:#555;text-decoration:underline;">Se désabonner</a></p>
</td></tr>
</table>
</td></tr>
</table>
